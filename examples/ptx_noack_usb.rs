...
async fn main(spawner: Spawner) {
    ...
    usb_config.product = Some("PTX NoAck");
    usb_config.serial_number = Some("PTXNOACK");
    ...
    let _ = class.write_packet(b"[PTX NoAck] Ready\r\n").await;
    ...
    loop {
        ...
        match ptx.send_no_ack(&payload).await {
            Ok(()) => queued += 1,
            Err(_) => tx_full += 1,
        }
        ...
        if counter % 200 == 0 && counter > 0 {
            let len = {
                ...
                let _ = write!(
                    w,
                    "[T] n={} q={} f={} m={}\r\n",
                    counter, queued, tx_full, max_attempts
                );
                w.pos
            };
